import os
import smtplib
import requests
from email.message import EmailMessage
from sqlalchemy import text
from urllib.parse import quote_plus
from sqlalchemy import create_engine
from datetime import datetime


# =====================================================================
# CONFIGURAÇÕES DE E-MAIL
# =====================================================================
EMAIL_REMETENTE = "nexusanalytics.br@telefonica.com"
SMTP_SERVIDOR = "10.128.11.229"
SMTP_PORTA = 25


# =====================================================================
# CONFIGURAÇÕES DE BANCO
# =====================================================================
DB_USER = "dev"
DB_PASSWORD = os.getenv("NOC_PASSWORD")
DB_HOST = "10.126.112.251"
DB_PORT = 3306
DB_NAME = "NEXUS"


def _engine():
    if not DB_PASSWORD:
        raise RuntimeError("NOC_PASSWORD não definida no ambiente.")
    pwd = quote_plus(DB_PASSWORD)
    return create_engine(
        f"mysql+pymysql://{DB_USER}:{pwd}@{DB_HOST}:{DB_PORT}/{DB_NAME}"
    )


# =====================================================================
# URL DO PORTAL NEXUS (AMBIENTE ATUAL)
# =====================================================================
URL_NEXUS_SOLICITACAO = (
    "https://10.8.141.212:8080/Administrador/Debora%20Teste/solicitante"
)


# =====================================================================
# CONFIGURAÇÃO DO TEAMS (POWER AUTOMATE)
# =====================================================================
POWER_AUTOMATE_URL = os.getenv("POWER_AUTOMATE_TEAMS_URL")


def enviar_teams_power_automate(id_solicitacao: int, mensagem: str, tipo_evento: str):
    if not POWER_AUTOMATE_URL:
        return

    try:
        requests.post(
            POWER_AUTOMATE_URL,
            json={
                "id_solicitacao": id_solicitacao,
                "tipo_evento": tipo_evento,
                "mensagem": mensagem,
            },
            timeout=5,
        )
    except Exception as e:
        print(f"[TEAMS] Erro ao enviar mensagem: {e}")


# =====================================================================
# MAPAS
# =====================================================================
STATUS_NOMES = {
    0: "Aberta",
    1: "Em análise",
    2: "Em andamento",
    3: "Concluída",
    4: "Cancelada",
}

PRIORIDADE_NOMES = {
    0: "Baixa",
    1: "Média",
    2: "Alta",
    3: "Crítica",
}

EVENTO_NOMES = {
    "CRIACAO": "Criação",
    "STATUS_ATUALIZADO": "Status atualizado",
    "PRIORIDADE_AREA": "Prioridade da área",
    "COMENTARIO_ADMIN": "Comentário do administrador",
    "PREVISAO_ENTREGA": "Previsão de entrega atualizada",
    "ARQUIVO_ENVIADO": "Arquivo enviado",
}


# =====================================================================
# FUNÇÃO PRINCIPAL
# =====================================================================
def enviar_notificacao_por_id(id_notificacao: int):
    eng = _engine()

    query = text("""
        SELECT 
            N.ID_NOTIFICACAO,
            N.ID_SOLICITACAO,
            N.TIPO_EVENTO,
            N.DESCRICAO_EVENTO,
            N.DATA_EVENTO,
            U.EMAIL AS EMAIL_DESTINO,
            U.NOME AS NOME_DESTINO
        FROM TBL_NOTIFICACOES_NEXUS N
        JOIN NOC.TBL_USUARIOS U
          ON U.MATRICULA = N.MATRICULA_DESTINO
        WHERE N.ID_NOTIFICACAO = :id
          AND N.ENVIADO = 0
    """)

    with eng.begin() as conn:
        row = conn.execute(query, {"id": id_notificacao}).mappings().first()

    if not row:
        return

    id_solic = row["ID_SOLICITACAO"]
    tipo = row["TIPO_EVENTO"]
    descricao = row["DESCRICAO_EVENTO"] or ""
    nome = row["NOME_DESTINO"]
    email = row["EMAIL_DESTINO"]

    data_fmt = row["DATA_EVENTO"].strftime("%d/%m/%Y %H:%M")

    # Tratamentos específicos
    if tipo == "STATUS_ATUALIZADO":
        try:
            n = int(descricao.split()[-1].replace(".", ""))
            descricao = f"Status alterado para <strong>{STATUS_NOMES.get(n)}</strong>."
        except Exception:
            pass

    if tipo == "PRIORIDADE_AREA":
        try:
            n = int(descricao.split()[-1].replace(".", ""))
            descricao = f"Prioridade da área alterada para <strong>{PRIORIDADE_NOMES.get(n)}</strong>."
        except Exception:
            pass

    assunto = f"[NEXUS] Atualização da Solicitação #{id_solic}"

    corpo_html = f"""
    <html>
    <body>
      <p>Olá, {nome}!</p>

      <p>Houve uma atualização na sua solicitação <strong>#{id_solic}</strong>.</p>

      <p><strong>Evento:</strong> {EVENTO_NOMES.get(tipo, tipo)}</p>
      <p><strong>Descrição:</strong> {descricao}</p>
      <p><strong>Data e hora do evento:</strong> {data_fmt}</p>

      <p>
        🔗 <a href="{URL_NEXUS_SOLICITACAO}" target="_blank">
        Acessar minha solicitação no Portal Nexus
        </a>
      </p>

      <p>Atenciosamente,<br>Equipe NEXUS</p>
    </body>
    </html>
    """

    msg = EmailMessage()
    msg["Subject"] = assunto
    msg["From"] = EMAIL_REMETENTE
    msg["To"] = email
    msg.set_content("Seu cliente de e-mail não suporta HTML.")
    msg.add_alternative(corpo_html, subtype="html")

    try:
        with smtplib.SMTP(SMTP_SERVIDOR, SMTP_PORTA) as smtp:
            smtp.send_message(msg)

        with eng.begin() as conn:
            conn.execute(
                text("UPDATE TBL_NOTIFICACOES_NEXUS SET ENVIADO = 1 WHERE ID_NOTIFICACAO = :id"),
                {"id": id_notificacao},
            )

        enviar_teams_power_automate(
            id_solic,
            f"Sua solicitação #{id_solic} foi atualizada.",
            tipo,
        )

    except Exception as e:
        with eng.begin() as conn:
            conn.execute(
                text("""
                    UPDATE TBL_NOTIFICACOES_NEXUS
                    SET TENTATIVAS = COALESCE(TENTATIVAS,0)+1,
                        ERRO_ULTIMO = :err
                    WHERE ID_NOTIFICACAO = :id
                """),
                {"id": id_notificacao, "err": str(e)},
            )
